...
Координатная система начинается с (0,0) в левом верхнем углу сетки.
...
- доступные команды
- текущая программа
- баллы, количество учеников, масштаб
...
Поле масштабируется через CSS-переменную `--scale`, которая задаётся ползунком «Масштаб поля». Масштаб влияет на размер клетки (`--cell-size`) и расстояние между клетками (`--cell-gap`), чтобы изменение происходило согласованно со всей вёрсткой.
